<x-admin-layout title="Create Blog Post">
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-bold font-outfit text-gray-900 leading-tight">Add New Blog Post</h2>
            </div>
    ...
    <div class="py-6">
        <form action="{{ route('admin.blogs.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-4xl">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 mb-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="2" required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-gold focus:ring-gold">{{ old('excerpt') }}</textarea>
                </div>

                <!-- Content -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Full Content</label>
                    <textarea name="content" rows="6" required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-gold focus:ring-gold">{{ old('content') }}</textarea>
                </div>
            ...
            <div class="flex justify-end pt-4 border-t border-gray-100">
                <button type="submit" class="bg-gold text-white px-6 py-2 rounded-lg font-bold shadow-md hover:bg-gold-600">Save Blog Post</button>
            </div>
        </form>
    </div>
