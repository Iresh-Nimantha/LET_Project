<x-admin-layout title="Edit Vision Highlight">
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-2xl font-bold font-outfit text-gray-900 leading-tight">
                    {{ __('Edit Highlight:') }} <span class="text-gold">{{ $visionHighlight->title }}</span>
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8">
                <form action="{{ route('admin.vision-highlights.update', $visionHighlight) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('title', $visionHighlight->title) }}" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('sort_order', $visionHighlight->sort_order) }}" required>
                                @error('sort_order') <span class="text-red-500 text-xs mt-2 block font-medium">{{ $message }}</span> @enderror
                            </div>

                            <div class="flex items-center mt-8">
                                <input type="checkbox" name="is_active" id="is_active" class="h-5 w-5 rounded border-gray-300 text-gold focus:ring-gold" {{ old('is_active', $visionHighlight->is_active) ? 'checked' : '' }}>
                        ...
                        <div>
                            <x-image-upload 
                                ...
                                helperText="PNG, JPG, WEBP. Uploading a new image will replace the current one."
                                :value="$visionHighlight->image_path"
                            />
                        ...
                        <div class="border-t border-gray-100 pt-6 mt-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="faq_question" id="faq_question" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('faq_question', $visionHighlight->faq_question) }}">
                                ...
                                <div>
                                    ...
                                    <textarea name="faq_answer" id="faq_answer" rows="3" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors">{{ old('faq_answer', $visionHighlight->faq_answer) }}</textarea>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-4">
                        ...
                        <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-gray-900 px-6 py-3 text-sm font-bold text-white shadow-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 transition-colors">
                            <i class="fa-solid fa-check mr-2"></i> Update Highlight
                        </button>
                    </div>
                </form>
            </div>
