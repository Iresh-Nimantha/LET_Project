                ...
                <form action="{{ route('admin.faqs.update', $faq) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="question" id="question" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('question', $faq->question) }}" required>
                        ...
                        <div>
                            ...
                            <textarea name="answer" id="answer" rows="5" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" required>{{ old('answer', $faq->answer) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('sort_order', $faq->sort_order) }}">
                            ...
                            <div class="flex items-center h-full pt-6">
                                <label class="flex items-center cursor-pointer">
                                    <div class="relative">
                                        <input type="checkbox" name="is_active" class="sr-only" value="1" {{ old('is_active', $faq->is_active) ? 'checked' : '' }} onchange="this.nextElementSibling.classList.toggle('bg-gold'); this.nextElementSibling.classList.toggle('bg-gray-200'); this.nextElementSibling.firstElementChild.classList.toggle('translate-x-full'); this.nextElementSibling.firstElementChild.classList.toggle('border-white');">
                                        <div class="block w-14 h-8 rounded-full transition-colors {{ old('is_active', $faq->is_active) ? 'bg-gold' : 'bg-gray-200' }}">
                                            <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition transform {{ old('is_active', $faq->is_active) ? 'translate-x-full border-white' : '' }}"></div>
                                        </div>
                                    </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-4">
                        ...
                        <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-gray-900 px-6 py-3 text-sm font-bold text-white shadow-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 transition-colors">
                            <i class="fa-solid fa-check mr-2"></i> Update FAQ
                        </button>
                    </div>
                </form>
            </div>
