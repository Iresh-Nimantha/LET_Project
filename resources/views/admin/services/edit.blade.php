<x-admin-layout title="Edit Service">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-2xl font-bold font-outfit text-gray-900 leading-tight">
                    {{ __('Edit Service:') }} <span class="text-gold">{{ $service->title }}</span>
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8">
                <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('title', $service->title) }}" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="icon_class" id="icon_class" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" value="{{ old('icon_class', $service->icon_class) }}">
                            @error('icon_class') <span class="text-red-500 text-xs mt-2 block font-medium">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label for="image" class="block text-sm font-bold text-gray-700 mb-2">Service Image <span class="text-gray-400 font-normal ml-2">(optional, leave blank to keep current)</span></label>
                            @if($service->image_path)
                                <div class="mb-3">
                                    <img src="{{ str_starts_with($service->image_path, 'data:image') ? $service->image_path : Storage::url($service->image_path) }}" alt="Current Image" class="h-20 w-20 object-cover rounded shadow-sm border border-gray-200">
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="5" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:bg-white focus:border-gold focus:ring-gold transition-colors" required>{{ old('description', $service->description) }}</textarea>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-4">
                        ...
                        <button type="submit" :disabled="isSubmitting" class="inline-flex items-center justify-center rounded-xl bg-gray-900 px-6 py-3 text-sm font-bold text-white shadow-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 transition-colors disabled:opacity-75 disabled:cursor-not-allowed">
                            <span x-show="!isSubmitting"><i class="fa-solid fa-check mr-2"></i> Update Service</span>
                            <span x-show="isSubmitting" x-cloak><i class="fa-solid fa-spinner fa-spin mr-2"></i> Updating...</span>
                        </button>
                    </div>
                </form>
            </div>
